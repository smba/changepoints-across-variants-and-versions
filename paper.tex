...
\begin{document}
	...
	{\color{teal}Our algorithm found multiple change points across parameter settings, including those with large numbers of commits and configuration options. While the search space of possible measurements is exponential in the number of configuration options, the theoretical maximum measurement budget before the algorithm aborted was 150,00 measurements, considering  5,000 measurements timeout of 30 iterations. The vast majority of cases terminated in early iterations with high accuracy. That is, we were able to tackle a problem with exponential complexity with a measurement budget that is linear in the number of iterations and the measurements per iteration, respectively.}
	...
	\end{comment}
	{\color{teal}In this paper, we have introduced a novel aproach to identify configuration-specific performance changes across software systems‘ version histories. We conducted two sets of experiments, in a controlled setup with synthesized performance data, and in a real-world setup with performance measurements of three configurable software systems. Our experiments with synthetic data show that our approach can identify configuration-specific software performance changes accurately and at scale. 
	Our results for real-wold software systems uphold the temporal precision of our approach. We could confirm prevalent practical challenges when studying real world-software systems and we were able to pinpoint performance changes to configuration options with little additional manual effort. It is an avenue for further research to incorporate further information, such as commit artifacts, when identifying configuration-specific software performance changes automatically.}
