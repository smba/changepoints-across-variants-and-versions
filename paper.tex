...
\begin{document}
	...
	%While the search space of possible measurements is exponential in the number of configuration options, the theoretical maximum measurement budget before the algorithm aborted was 150,00 measurements, considering  5,000 measurements timeout of 30 iterations. 
	...
	Inconsistencies in the documentation of configuration options is a well known and prevalent problem~\cite{rabkin_static_2011}. 
	...
	\section{Conclusion}
	Changes in the software performance of software systems can be configuration-specific and are impossible to unveil with exhaustive testing. In retrospect, knowledge about previous and existing performance changes can be useful to prioritize future measurements for performance regression testing. 
	We take a look at both the commit history and configurability of software systems and propose a novel approach to identify and pinpoint performance changes to individual commits and configuration options and interactions. Our approach leverages similarities in performance histories of related configurations to identify performance changes. Subsequently, it associates such performance changes to individual configuration options and interactions. Our approach starts with a small set of measurements and iteratively queries new measurements to refine a prediction of configuration-specific change points.
	We have shown with synthetic data that our approach is sound and can identify configuration-specific performance changes accurately and at scale. In a practical setting, we face prevalent challenges that can be handled with little manual effort. It is an avenue for further research to incorporate information, such as commit artifacts, into pinpointing performance changes to configuration options and interactions automatically. Our approach opens a new combined perspective by the combined analysis of software performance histories as well as software configuration spaces.
	
	\section{Acknowledgments}
	We would also like to thank our reviewers for their comments. Apel’s work has been supported by the Deutsche Forschungsgemeinschaft (DFG) under the contract AP 206/11-1. Siegmund’s work has been supported by the DFG under the
