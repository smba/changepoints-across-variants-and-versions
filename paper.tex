...
\begin{document}
	...
	\section{Conclusion}
	During software evolution changes can introduce performance
	bottlenecks that are challenging detecting. As most contemporary
	software systems are configurable, such performance changes are
	often configuration-specific. This renders detecting configuration-
	specific performance changes impossible with exhaustive measurements due to the combinatorial complexity of software systems‘ configuration spaces. We propose a novel approach to tackle
	this complexity and identify configuration-specific performance
	changes with workable effort. Starting from a small sample of measurements (pairs of configurations and commits), our approach iteratively selects new measurements along two dimensions: time and
	configuration space. To iteratively update an approximation of existing change points, the selection of new measurements is guided by
	an estimated change point likelihood of a commit introducing a performance change as well as by configurations that are likely associated
	with performance changes, both based on previous measurements.
	We evaluate our approach with respect to scalability, accuracy, and practicality with two sets of experiments. To demonstrate that our approach is sound
	and efficient, we have applied our approach to synthetic data of
	software systems varying in version history size and configuration
	complexity. With relatively few measurements, we can accurately
	identify configuration-specific performance changes for software
	systems which exhibit thousands of commits and millions of configurations. To demonstrate feasibility in a practical setting, we
	applied our approach to performance data of three configurable
	software systems. We were able to detect prevalent performance
	changes accurately, while partially confirming prevalent practical
	challenges that are easy to overcome with manual inspection of
	commit messages. In retrospect, our approach provides knowledge about previous and existing performance changes which can be useful to
	prioritize future measurements for performance regression testing.
